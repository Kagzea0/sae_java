...
public class TextAreaScenario22 extends GridPaneAPLI {
    TextArea textArea;
    Button bouton;

    TextArea textAreaParcours;

    TextAreaScenario22() throws IOException {
        textArea = new TextArea();
        Scenario scenario = new Scenario();
        File[] fichiersScenarios = new File("Scenario").listFiles();
        textArea.appendText(scenario.affichageScenario(fichiersScenarios[4]));
        this.add(textArea,8,4,4,10);
        bouton = new Button("<");
        this.add(bouton,0,0,2,1);
        menu.setText("Scénario 2.2");
        textAreaParcours = new TextArea();
        this.add(textAreaParcours, 8, 16, 4, 4);
        String str = "Parcours du scénario 2.2 : ";
        ArrayList listeScenario = scenario.parcours(scenario.dictionnaire(fichiersScenarios[4]), LectureScenario.lectureScenario(fichiersScenarios[4]).getVendeurs(), LectureScenario.lectureScenario(fichiersScenarios[4]).getAcheteurs(),fichiersScenarios[4]);
